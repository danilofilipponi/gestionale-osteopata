...
const DOCTOR_NAME = "Filipponi Danilo";
const DOCTOR_TITLE = "Riabilitazione - Osteopatia";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.race([
      ...
      .then(({ data }) => { if (!cancelled) setLoggedIn(Boolean(data.session)); })
...
function Logo({ inverse = false }: { inverse?: boolean }) {
  return <div className={`flex items-center gap-3 ${inverse ? "text-white" : "text-ink"}`}><span className={`grid h-14 w-14 place-items-center overflow-hidden rounded-2xl ${inverse ? "bg-white" : "border border-line bg-white"}`}><img src="/logo-filipponi.jpg" alt="Logo Filipponi Danilo" className="h-full w-full object-contain"/></span><div><b className="text-lg">{DOCTOR_NAME}</b><small className={`block text-[10px] font-bold uppercase tracking-[.18em] ${inverse ? "text-[#acd1ca]" : "text-muted"}`}>{DOCTOR_TITLE}</small></div></div>;
}
...
function Sidebar({ view, go, logout, open }: { view: View; go: (v: View) => void; logout: () => void; open: boolean }) {
  ...
  return <aside className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-line bg-white p-5 transition-transform md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
    ...
    <div className="mt-auto rounded-2xl bg-mist p-4"><p className="text-xs font-bold uppercase tracking-wide text-muted">Account</p><p className="mt-1 text-sm font-bold">{DOCTOR_NAME}</p><button onClick={logout} className="mt-4 flex items-center gap-2 text-sm font-bold text-muted hover:text-red-600"><Icon name="logout" size={16}/> Esci</button></div>
  </aside>;
}
...
function Header({ view, onMenu }: { view: View; onMenu: () => void }) {
  ...
  return <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-line bg-white/90 px-5 backdrop-blur md:px-8"><div className="flex items-center gap-3"><button className="md:hidden" onClick={onMenu}><Icon name="menu"/></button><div><h1 className="text-xl font-bold">{title}</h1><p className="hidden text-xs text-muted sm:block">Mercoledi, 3 giugno 2026</p></div></div><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><b className="block text-sm">{DOCTOR_NAME}</b><small className="text-muted">{DOCTOR_TITLE}</small></div><div className="grid h-10 w-10 place-items-center rounded-full bg-[#dbeae7] text-sm font-bold text-sage">{DOCTOR_INITIALS}</div></div></header>;
}
...
function Dashboard({ patients, invoices, go, newPatient, newInvoice }: { patients: Patient[]; invoices: Invoice[]; go: (v: View) => void; newPatient: () => void; newInvoice: () => void }) {
  ...
  return <><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-muted">Buongiorno, Danilo</p><h2 className="mt-1 text-3xl font-bold">Il tuo studio, oggi.</h2></div><div className="flex gap-2"><Button variant="secondary" icon="plus" onClick={newPatient}>Nuovo paziente</Button><Button icon="plus" onClick={newInvoice}>Nuova fattura</Button></div></div>
...
function PatientModal({ value, close, save }: { value: Patient; close:()=>void; save:(p:Patient)=>void }) {
  const [p, setP] = useState(value);
  const [taxCodeManual, setTaxCodeManual] = useState(Boolean(value.taxCode));
  const applyAutomaticTaxCode = (patient: Patient) => {
    const code = calculateTaxCode(patient);
    return code && !taxCodeManual ? { ...patient, taxCode: code } : patient;
  };
  const update = (key: keyof Patient, rawValue: string) => {
    let next = { ...p, [key]: rawValue };
    if (key === "city") {
      const city = findCity(rawValue);
      if (city) next = { ...next, province: city.province, zip: city.zip };
    }
    if (["firstName", "lastName", "birthDate", "gender", "birthCity"].includes(key)) {
      next = applyAutomaticTaxCode(next);
    }
    setP(next);
  };
  const setManualTaxCode = (value: string) => {
    setTaxCodeManual(true);
    setP({ ...p, taxCode: value.toUpperCase() });
  };
  const computeTaxCode = () => {
    const code = calculateTaxCode(p);
    if (code) {
      setTaxCodeManual(false);
      setP({ ...p, taxCode: code });
    } else {
      alert("Per calcolare il codice fiscale compila nome, cognome, sesso, data di nascita e un comune di nascita presente nell'elenco.");
    }
  };
  return <Modal title={p.id?"Modifica paziente":"Nuovo paziente"} close={close}><datalist id="italian-cities">{italianCities.map(c=><option key={c.name} value={c.name}/>)}</datalist><div className="grid gap-4 sm:grid-cols-2"><Input label="Nome *" value={p.firstName} onChange={e=>update("firstName",e.target.value)}/><Input label="Cognome *" value={p.lastName} onChange={e=>update("lastName",e.target.value)}/><Input label="Telefono" value={p.phone} onChange={e=>update("phone",e.target.value)}/><Input label="Email" value={p.email} onChange={e=>update("email",e.target.value)}/><Input label="Professione" value={p.profession} onChange={e=>update("profession",e.target.value)}/><Input label="Data di nascita" type="date" value={p.birthDate} onChange={e=>update("birthDate",e.target.value)}/><Select label="Sesso" value={p.gender} onChange={e=>update("gender",e.target.value)}><option>F</option><option>M</option><option>Altro</option></Select><Input label="Comune di nascita" list="italian-cities" value={p.birthCity} onChange={e=>update("birthCity",e.target.value)} placeholder="Cerca comune..."/><div><Input label="Codice fiscale" value={p.taxCode} onChange={e=>setManualTaxCode(e.target.value)} placeholder="Calcolo automatico o modifica manuale"/><button type="button" onClick={computeTaxCode} className="mt-2 text-xs font-bold text-sage">Ricalcola automaticamente</button></div><Input label="Indirizzo" value={p.address} onChange={e=>update("address",e.target.value)}/><Input label="Citta di residenza" list="italian-cities" value={p.city} onChange={e=>update("city",e.target.value)} placeholder="Cerca comune..."/><Input label="Provincia" value={p.province} onChange={e=>update("province",e.target.value)}/><Input label="CAP" value={p.zip} onChange={e=>update("zip",e.target.value)}/></div><div className="mt-6 flex justify-end gap-2"><Button variant="secondary" onClick={close}>Annulla</Button><Button onClick={()=>p.firstName&&p.lastName&&save(p)}>Salva paziente</Button></div></Modal>;
}
