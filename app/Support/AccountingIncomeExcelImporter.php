...
class AccountingIncomeExcelImporter
{
    public static function importAnnualSummary(UploadedFile $file, int $year): int
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($grossIncome === null && $totalIncome !== null && $invoiced !== null) {
                $grossIncome = max($totalIncome - $invoiced, 0);
            }

            if ($month === null || ($invoiced === null && $grossIncome === null)) {
                continue;
            }

            AccountingIncomeSummary::updateOrCreate(
                ['user_id' => Auth::id(), 'year' => $year, 'month' => $month],
                ['invoiced_amount' => $invoiced, 'gross_income_amount' => $grossIncome],
            );
    ...
    public static function importYear(UploadedFile $file, int $year): int
    {
        ...
        foreach ($rows as $row) {
            $date = self::dateValue(self::valueAny($row, $indexes, ['data', 'data entrata', 'data incasso', 'date']));
            $month = self::monthValue(self::valueAny($row, $indexes, ['mese', 'month']), $date);
            $amount = self::amountValue(self::valueAny($row, $indexes, ['importo', 'totale', 'entrata', 'incasso', 'amount']));
            $description = self::valueAny($row, $indexes, ['causale', 'descrizione', 'cliente', 'dettaglio', 'description']);

            if (($amount === null && blank($description) && blank($date)) || $month === null) {
                continue;
            }

            AccountingIncome::create([
                'user_id' => Auth::id(),
                'year' => $year,
                'month' => $month,
                'income_date' => $date,
                'description' => $description ?: 'Entrata importata',
                'amount' => $amount ?? 0,
            ]);
    ...
    private static function rows(string $path): array
    {
        ...
        if ($zip->open($path) !== true) {
            throw new RuntimeException('File Excel entrate non leggibile.');
        }
        ...
        if ($xml === false) {
            throw new RuntimeException('Foglio entrate non trovato.');
        }
    ...
    private static function amountValue(string $value): ?float
    {
        ...
        $normalized = str_replace(['EUR', '€', 'â‚¬', ' '], '', $value);
