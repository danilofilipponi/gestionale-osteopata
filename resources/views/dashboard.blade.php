    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mt-6 grid gap-6 lg:grid-cols-2">
                ...
                <section class="rounded-lg bg-white p-6 shadow-sm">
                    <h3 class="font-semibold text-gray-900">Prossime sedute</h3>
                    <div class="mt-4 divide-y divide-gray-100">
                        @forelse ($upcomingSessions as $session)
                            <a href="{{ route('patients.show', $session->patient) }}" class="block py-3">
                                <div class="flex items-center justify-between gap-4">
                                    <span class="font-medium text-gray-900">{{ $session->patient->full_name }}</span>
                                    <span class="text-sm text-gray-500">{{ $session->session_date->format('d/m/Y') }}</span>
                                </div>
                                <p class="mt-1 text-sm text-gray-600">{{ $session->title }}</p>
                            </a>
                        @empty
                            <p class="py-6 text-sm text-gray-500">Nessuna seduta programmata.</p>
                        @endforelse
                    </div>
                </section>
            </div>
